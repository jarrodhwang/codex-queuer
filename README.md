...
`model_context_window` controls Codex's token accounting and compaction threshold. For Ollama, Codex Queue also creates and reuses an internal derived model with the selected value stored as `num_ctx`, so the request dropdown changes the actual Ollama allocation. LM Studio and llama.cpp do not expose the same model-creation API; configure those backends with an equal or larger context window. Contexts from 4K through 256K are supported and each Local run records its applied size in the work log. Smaller windows save accelerator memory and can suit short prompts, but 32K or higher is recommended for project work because tool results and file contents quickly consume the available context.
