...
Codex Queue supports 32,768 tokens as the OpenHands minimum and now defaults its Local Ollama profile to 131,072 tokens (128K), the advertised maximum for GPT-OSS. It sends the selected request value to Ollama as `options.num_ctx`. To let Ollama load that maximum for other clients too, set `OLLAMA_CONTEXT_LENGTH=131072`, restart Ollama, and verify the loaded allocation with `ollama ps`. A model whose advertised maximum is lower remains visible in the plain model list and is capped to that model's maximum. `/api/tags` reports the model's maximum, whereas `/api/ps` reports the currently loaded allocation. Context memory grows substantially with the window, so use 64K or 32K if the server reports allocation failures or becomes slow. See the official [OpenHands local-LLM guide](https://docs.openhands.dev/openhands/usage/llms/local-llms), [Ollama context-length guide](https://docs.ollama.com/context-length), and [Ollama OpenAI compatibility documentation](https://docs.ollama.com/api/openai-compatibility).
