...
Sandboxed Codex runs over Windows SSH use the native unelevated sandbox on the SSH session's compatible `Winsta0\\Default` desktop. This avoids elevated sandbox setup, sandbox-user logon failures, and private-desktop process initialization failures in non-interactive SSH sessions while retaining ACL-based workspace boundaries. The trade-offs are weaker network and UI isolation than the elevated sandbox with its private desktop. Local Windows runs continue to use the Codex-configured sandbox mode and desktop.
